...
export class AppHome extends LitElement {
  ...
  private async connectToRoom(roomName: string) {
    const response = await fetch('/api/livekit-token', {
      ...
      body: JSON.stringify({ room: roomName, participantName: this.displayName('Jordan Davis') }),
    });
    ...
    return html`
      <div class="app-shell theme-${this.colorTheme}">
        ...
        <main class="chat-panel">
          ...
          <section class="messages" aria-live="polite"><div class="day-divider"><span>Today</span></div>${this.messages.map((message) => html`<article class="message ${message.own ? 'own' : ''}"><span class="avatar avatar-${message.own ? 'you' : '1'}">${message.own ? 'JD' : 'MC'}</span><div class="message-body"><div class="message-meta"><strong>${this.displayName(message.author)}</strong><time>${message.time}</time></div><p>${message.text}</p></div></article>`)}</section>
